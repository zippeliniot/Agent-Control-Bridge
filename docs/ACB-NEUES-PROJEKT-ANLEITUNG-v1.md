...
| 7 | PowerShell, dann claude.ai | Im ACB-Klon des Projekts `.venv\Scripts\python.exe scripts\steuerchat-vorlage.py --project-id <id>` ausführen (der Steuerchat liefert ihn mit konkretem Wert). Neues claude.ai-Projekt anlegen, die komplette Ausgabe als Custom Instructions einfügen. | Projekt-Steuerchat bereit |
